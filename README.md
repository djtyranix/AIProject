...
## Uninformed Search
Untuk uninformed search, akan seperti pada 8-puzzle, dimana program akan mencari goal state dengan cara brute force, tanpa adanya pengetahuan tentang sesuatu layaknya informed search. Dalam BFS, mulai dari random initial state, maka akan terus dicari goal state dengan membuat suatu search tree untuk current dan next state. Ketika sudah mendapatkan bahwa salah satu node adalah goal state, maka pencarian akan berhenti dan mengembalikan goal state tanpa perlu tracking down jalur untuk menuju ke state tersebut.

## Hill-Climbing Search
Dalam hill-climbing search yang merupakan local search, akan diambil satu state yang menjadi initial state. Untuk sementara, initial state ini merupakan goal state. Secara garis besar, hill climbing adalah local search dimana algoritmanya akan mencari kemungkinan terbaik sampai tidak ada lagi kemungkinan yang lebih baik daripada state yang sekarang. Program akan menghitung seberapa banyak kemungkinan serang dari tiap queen yang ada lalu di jumlahkan. Hasil dari jumlah tersebut akan dimisalkan sebagai queenKill. Ketika queenKill semakin kecil nilainya, maka state akan semakin mendekati goal state yang diinginkan, dimana seharusnya queenKill == 0. Program akan membuat kemungkinan state selanjutnya berdasarkan value pada queenKill. Ketika program menemukan next state yang nilai queenKillnya lebih kecil dari nilai queenKill yang sekarang, maka program akan mengganti state yang sekarang ke state tersebut dan begitu seterusnya sampai mencapai goal state.

## Constraint Satisfaction Problem (CSP)
Dalam cara penyelesaian ini, n-Queen dianggap sebagai suatu CSP, yaitu problem yang dimana constraintnya harus dipenuhi dengan cara cara tertentu. Dalam CSP juga dikenal adanya variabel, domain, dan constraint. Tujuan CSP sendiri adalah mengisi setiap variabel dengan domain yang telah ditentukan sehingga semua contraint dapat terpenuhi. Dalam hal ini, akan digunakan contraint propagation. Dalam 4-Queen, terdapat 4 x 4 board. Ini berarti, ada 16 tiles. Misalkan, akan ditaruh queen pertama secara random, biasanya dimulai dari pojok kiri atas. Karena kita tahu bahwa queen bisa bergerak lurus ke arah horizontal, vertikal, maupun diagonal, maka akan dibuat sebuah pemetaan pada game board untuk tempat tempat yang tidak dapat di tuju oleh queen yang pertama. Anggap bahwa tempat aman ini diberi warna hijau dan tidak aman diberi warna hitam. Queen ke-2 akan di letakkan di sebarang tiles berwarna hijau. Maka, tempat yang berwarna hijau pada map akan berubah mengikuti kondisi game board sekarang. Hal ini akan dilakukan sampai queen ke 4 berhasil di letakkan dalam game board. Jika tidak ada tempat hijau tersisa, maka akan kembali ke state sebelumnya dan meletakkan ulang queen sebelumnya di posisi yang berbeda.

# Tic-Tac-Toe
Tic-Tac-Toe adalah sebuah permainan klasik dimana permainan ini akan dimainkan oleh 2 player. Setiap player memiliki lambang masing masing, yang biasanya adalah 'X' dan 'O'. Goal state dari permainan ini adalah salah satu player dapat membuat garis berisi 3 lambang mereka. Jika board penuh dan tidak ada yang berhasil memenangkan game, maka akan seri. AI disini akan menggunakan algoritma minimax, dimana algoritma ini sangat cocok untuk permainan Tic-Tac-Toe.
